...
`gen.Battle()`은 Go query factory이며 `*gen.BattleQuery`를 반환한다. PHP와 Rust는 각각 `Battle::query()`와 `battle::query()`를 사용한다. query operation과 의미는 공통이고 생성 문법, 소유권, 비동기 실행은 언어 규칙을 따른다. `get`은 반드시 한 행을 반환하며 결과가 없으면 `NO_ROWS`를 반환한다. 선택적 조회가 필요하면 명시적인 `getOrNil`을 사용한다. `gets`는 collection, `getCount`는 scalar count를 반환한다. 실행 전에 `using`으로 executor를 지정한다. Go는 DSN으로 생성한 database가 실행 context를 보유하므로 호출자는 database 또는 transaction executor만 전달한다.
