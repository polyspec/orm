...
// Go generator: one file per entity in the requested package, against clients/go/orm.
...
func relationText(m *schema.Manifest, owner *schema.Entity, r *schema.Rel) (left, right, suffix string) {
	...
	for i, key := range r.Keys {
		leftColumns[i], rightColumns[i] = key.Local, key.Target
		parts[i] = pascal(key.Local) + "With" + pascal(key.Target)
	}
	return strings.Join(leftColumns, ","), strings.Join(rightColumns, ","), strings.Join(parts, "And")
}
...
func buildGoEntity(m *schema.Manifest, e *schema.Entity) goEntity {
	ge := goEntity{Name: e.Name, Type: pascal(e.Name), Table: e.Table, PK: e.PK[0], PKMethod: finderMethod(e.PK), PKNames: append([]string(nil), e.PK...), Auto: e.Auto != "", AutoCol: e.Auto}
	hasAES := false
	...
	for _, cols := range e.Unique {
		...
		if seenUnique[key] || len(cols) == 0 || strings.Join(cols, "\x1f") == strings.Join(ge.PKNames, "\x1f") {
			continue
		}
		seenUnique[key] = true
	...
	for _, pn := range m.Order {
		...
		for _, r := range pe.Relations {
			...
			linkPairs[key] = true
			ge.Links = append(ge.Links, goLink{Left: left, Right: right, Match: "Match" + suffix, On: "On" + suffix})
		}
	}
...
var goTmpl = template.Must(template.New("go").Funcs(template.FuncMap{
	...
	"fieldValues": func(fields []goCol, prefix string) string {
		values := make([]string, len(fields))
		for i, field := range fields {
			values[i] = prefix + field.Field
		}
...
// {{.Type}}Key contains the complete ordered primary key of {{.Table}}.
type {{.Type}}Key struct {
{{- range .PKCols}}
	{{.Field}} {{.Type}}
{{- end}}
}
...
{{- end}}{{end}}
	...
	r.Mark({{printf "%q" .Name}}, []string{ {{quoteList .PKNames}} }, []any{ {{fieldValues .PKCols "r."}} })
{{- if .UpdatedTs}}
...
func (q *{{.Type}}Query) AESStatus(keyring orm.AESKeyring) (orm.AESRotationStatus, error) {
	...
	return ex.DB().AESStatus(ctx, ex, orm.AESRotationSpec{Table: {{printf "%q" .Table}}, PrimaryKeys: []string{ {{quoteList .PKNames}} }, VersionColumn: {{printf "%q" .AESVersion}}}, keyring)
}
...
func collect{{.Type}}Direct(rows []*{{.Type}}Row, keyFn func(*{{.Type}}Row) orm.Key) *orm.Collection[{{.Type}}Row] {
	...
	for _, r := range rows {
		if keyFn != nil { c.Put(keyFn(r), r) } else { c.Put(orm.KeyFromValues([]any{ {{fieldValues .PKCols "r."}} }), r) }
	}
	return c
}

// GetsAfter reads the next keyset page. An empty cursor starts at the first row.
func (q *{{.Type}}Query) GetsAfter(cursor string, per int) (*orm.KeysetPage[{{.Type}}Row], error) {
	if err := q.q.Keyset("after", cursor, per, []string{ {{quoteList .PKNames}} }); err != nil { return nil, err }
	ctx, ex, err := q.binding.Resolve(); if err != nil { return nil, err }
	q.q.Req.IR.Kind = "all"
	rows, err := orm.Query(ctx, ex, q.q.Req); if err != nil { return nil, err }
	items := collect{{.Type}}(rows, q.keyFn)
...
func (q *{{.Type}}Query) GetsBefore(cursor string, per int) (*orm.KeysetPage[{{.Type}}Row], error) {
	if err := q.q.Keyset("before", cursor, per, []string{ {{quoteList .PKNames}} }); err != nil { return nil, err }
	ctx, ex, err := q.binding.Resolve(); if err != nil { return nil, err }
	q.q.Req.IR.Kind = "all"
	rows, err := orm.Query(ctx, ex, q.q.Req); if err != nil { return nil, err }
...
// GetBy{{.PKMethod}} applies every primary-key component in declared order.
func (q *{{.Type}}Query) GetBy{{.PKMethod}}({{finderParams .PKCols}}) (*{{.Type}}Row, error) {
	return {{finderChain .PKCols}}.Get()
}
{{- end}}
{{range .UniqueFinders}}
...
func genGo(m *schema.Manifest, outDir string, packageNames ...string) error {
	packageName := "gen"
	...
	if err := os.WriteFile(filepath.Join(outDir, "orm_init.go"), []byte(fmt.Sprintf(goInitFile, packageName, m.SchemaHash)), 0o644); err != nil {
		return err
	}
	for _, name := range m.Order {
		e := m.Entities[name]
		ge := buildGoEntity(m, e)
		ge.Package = packageName
...
func GenerateGo(m *schema.Manifest, outDir string, packageName ...string) error {
	return genGo(m, outDir, packageName...)
}
