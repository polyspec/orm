...
func genTypeScript(m *schema.Manifest, outDir string) error {
	...
	b.WriteString("import { Collection, KeysetPage, Page, Row, keysetPage, registerRow, rowCollection } from '../model.js';\n")
	...
	for _, name := range m.Order {
		...
		for _, rel := range ge.Rels {
			...
			fmt.Fprintf(&b, "  public count%sEq(value: number, callback: (where: %sWhere) => void): this { this.core.navigateCount(%s,'eq',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sGte(value: number, callback: (where: %sWhere) => void): this { this.core.navigateCount(%s,'gte',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sGt(value: number, callback: (where: %sWhere) => void): this { this.core.navigateCount(%s,'gt',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sLte(value: number, callback: (where: %sWhere) => void): this { this.core.navigateCount(%s,'lte',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sLt(value: number, callback: (where: %sWhere) => void): this { this.core.navigateCount(%s,'lt',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sNotEq(value: number, callback: (where: %sWhere) => void): this { this.core.navigateCount(%s,'not_eq',value,core=>callback(new %sWhere(core))); return this; }\n", rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType)
		}
		b.WriteString("}\n\n")
		...
		for _, rel := range ge.Rels {
			...
			fmt.Fprintf(&b, "  public count%sEq(value: number, callback: (where: %sWhere) => void): this { this.whereCore().navigateCount(%s,'eq',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sGte(value: number, callback: (where: %sWhere) => void): this { this.whereCore().navigateCount(%s,'gte',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sGt(value: number, callback: (where: %sWhere) => void): this { this.whereCore().navigateCount(%s,'gt',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sLte(value: number, callback: (where: %sWhere) => void): this { this.whereCore().navigateCount(%s,'lte',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sLt(value: number, callback: (where: %sWhere) => void): this { this.whereCore().navigateCount(%s,'lt',value,core=>callback(new %sWhere(core))); return this; }\n  public count%sNotEq(value: number, callback: (where: %sWhere) => void): this { this.whereCore().navigateCount(%s,'not_eq',value,core=>callback(new %sWhere(core))); return this; }\n", rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType, rel.Method, rel.TargetType, tsString(rel.Name), rel.TargetType)
		}
		for _, rel := range ge.Rels {
			suffix := rel.Suffix
		...
		b.WriteString("  public async getsAfter(cursor: string, per: number): Promise<KeysetPage<" + ge.Type + "Row>> { this.keyset('after',cursor,per,[" + strings.Join(pkNames, ",") + "]); const rows=await this.terminalRows(); const items=rowCollection<" + ge.Type + "Row>(rows, rows.plan.steps[0]!.assemble!); return keysetPage(rows,items,this.request.ir.order ?? []); }\n")
		b.WriteString("  public async getsBefore(cursor: string, per: number): Promise<KeysetPage<" + ge.Type + "Row>> { this.keyset('before',cursor,per,[" + strings.Join(pkNames, ",") + "]); const rows=await this.terminalRows(); for(let left=0,right=rows.data.length-1;left<right;left++,right--){ const value=rows.data[left]!; rows.data[left]=rows.data[right]!; rows.data[right]=value; } const items=rowCollection<" + ge.Type + "Row>(rows, rows.plan.steps[0]!.assemble!); return keysetPage(rows,items,this.request.ir.order ?? []); }\n")
