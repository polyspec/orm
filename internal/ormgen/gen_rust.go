...
{{- range .Rels}}
    ...
    pub fn count_{{.Ident}}_eq(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.w.nav_with_count({{printf "%q" .Name}}, "eq", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_gte(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.w.nav_with_count({{printf "%q" .Name}}, "gte", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_gt(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.w.nav_with_count({{printf "%q" .Name}}, "gt", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_lte(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.w.nav_with_count({{printf "%q" .Name}}, "lte", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_lt(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.w.nav_with_count({{printf "%q" .Name}}, "lt", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_not_eq(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.w.nav_with_count({{printf "%q" .Name}}, "not_eq", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
{{- end}}
...
{{- range .Rels}}
    ...
    pub fn count_{{.Ident}}_eq(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.q.w().nav_with_count({{printf "%q" .Name}}, "eq", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_gte(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.q.w().nav_with_count({{printf "%q" .Name}}, "gte", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_gt(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.q.w().nav_with_count({{printf "%q" .Name}}, "gt", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_lte(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.q.w().nav_with_count({{printf "%q" .Name}}, "lte", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_lt(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.q.w().nav_with_count({{printf "%q" .Name}}, "lt", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
    pub fn count_{{.Ident}}_not_eq(mut self, value: i64, f: impl FnOnce(super::{{.Target}}::{{.TargetType}}Where<'_>) -> super::{{.Target}}::{{.TargetType}}Where<'_>) -> Self { self.q.w().nav_with_count({{printf "%q" .Name}}, "not_eq", value, |w| { f(super::{{.Target}}::{{.TargetType}}Where { w }); }); self }
{{- end}}
{{range .Cols}}{{$c := .}}{{range .Ops}}
{{- if eq .Kind "one"}}
...
{{- end}}{{end}}
    ...
    pub async fn gets_after(&mut self, cursor: &str, per: u32) -> Result<orm::KeysetPage<{{.Type}}Row>> {
        self.q.keyset("after", cursor, per, &[{{rsList .KeyCols}}])?;
    ...
    pub async fn gets_before(&mut self, cursor: &str, per: u32) -> Result<orm::KeysetPage<{{.Type}}Row>> {
        self.q.keyset("before", cursor, per, &[{{rsList .KeyCols}}])?;
        let binding = self.binding.clone(); let ex = binding.resolve()?;
        let mut rows = db::select(ex, &mut self.q.req, "all").await?;
        rows.reverse_main();
        let (next_cursor, previous_cursor) = rows.keyset_cursors(&self.q.req.ir.query.order)?;
...
{{- end}}
    ...
    pub async fn batch_insert(&self, rows: Vec<{{.Type}}>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "insert", options).await }
    pub async fn batch_upsert(&self, rows: Vec<{{.Type}}>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "insert", options).await }
    pub async fn batch_update(&self, rows: Vec<{{.Type}}>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "update", options).await }
    pub async fn batch_delete(&self, rows: Vec<{{.Type}}>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "delete", options).await }
    async fn batch_write(&self, rows: Vec<{{.Type}}>, kind: &str, options: orm::BatchOptions) -> Result<orm::BatchResult> {
        let binding = self.binding.clone(); let ex = binding.resolve()?;
...
//! Generated entities. Call \x60connect()\x60 once, then use \x60battle::query()\x60 etc.
#![allow(clippy::all)]
