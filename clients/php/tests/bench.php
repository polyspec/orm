...
bench('client pk one', $iters, function ($i) use ($db) { $r = Battle::query()->seqEq($i % 100000 + 1)->using($db)->one(); if ($r === null) throw new RuntimeException('no row'); });
bench('client list100', $iters, function ($i) use ($db) { $c = Battle::query()->serviceSeqEq($i % 100 + 1)->isCloseEq(false)->orderBySeqDesc()->limit(0, 100)->using($db)->all(); if (count($c) === 0) throw new RuntimeException('empty'); });
bench('client list100 + getName x100', $iters, function ($i) use ($db) { $c = Battle::query()->serviceSeqEq($i % 100 + 1)->isCloseEq(false)->orderBySeqDesc()->limit(0, 100)->using($db)->all(); foreach ($c as $r) { $r->getName(); } });
