...
bench('client pk one', $iters, function ($i) use ($db) { $r = (new Battle)->seqEq($i % 100000 + 1)->bind($db)->one(); if ($r === null) throw new RuntimeException('no row'); });
bench('client list100', $iters, function ($i) use ($db) { $c = (new Battle)->serviceSeqEq($i % 100 + 1)->isCloseEq(false)->orderBySeqDesc()->limit(0, 100)->bind($db)->all(); if (count($c) === 0) throw new RuntimeException('empty'); });
bench('client list100 + getName x100', $iters, function ($i) use ($db) { $c = (new Battle)->serviceSeqEq($i % 100 + 1)->isCloseEq(false)->orderBySeqDesc()->limit(0, 100)->bind($db)->all(); foreach ($c as $r) { $r->getName(); } });
