...
final class User extends Q implements UserInterface
{
    ...
    public function getOrNull(): ?UserRow
    {
    ...
    public function getsAfter(string $cursor, int $per): KeysetPage
    {
        ...
        return KeysetPage::fromRows($rows, Collection::fromRows($rows, UserRow::class, $this->keyFn), $this->req->ir['order'] ?? []);
    }
    ...
    /** @param list<User> $rows */
    private function batchWrite(array $rows, string $kind, ?BatchOptions $options): BatchResult
    {
        $db = $this->terminalDb(); $requests = [];
        foreach ($rows as $row) {
            if (!$row instanceof User) throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'batch row has the wrong type');
