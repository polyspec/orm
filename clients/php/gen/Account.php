...
final readonly class AccountKey
{
    public function __construct(public int $seq) {}
}

/** One row of account. */
final class AccountRow extends Row implements AccountRowInterface
{
    public static function entity(): string { return 'account'; }
    ...
    {
        return ['projects' => ['kind' => 'many', 'target' => 'project', 'left' => 'seq', 'right' => 'seq']];
    }
    ...
    public function getProjects(): Collection { return $this->relation('projects') ?? new Collection(); }
}

/** Column references for column-to-column predicates (AccountCols::seq()); ->at('service') points into a joined entity. */
final class AccountCols
{
    public static function seq(): ColRef { return new ColRef('seq'); }
    public static function name(): ColRef { return new ColRef('name'); }
}

/** Where builder for account: typed predicates, or(), and(fn), and relation navigation. */
final class AccountWhere
{
    ...
    public function projects(\Closure $fn): static { $fn(new ProjectWhere($this->w->nav('projects'))); $this->w->req->end(); return $this; }
    public function hasProjects(\Closure $fn): static { $fn(new ProjectWhere($this->w->navMode('projects', 'exists'))); $this->w->req->end(); return $this; }
    public function notHasProjects(\Closure $fn): static { $fn(new ProjectWhere($this->w->navMode('projects', 'not_exists'))); $this->w->req->end(); return $this; }
    public function countProjectsEq(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w->navCount('projects', 'eq', $value))); $this->w->req->end(); return $this; }
    public function countProjectsGte(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w->navCount('projects', 'gte', $value))); $this->w->req->end(); return $this; }
    public function countProjectsGt(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w->navCount('projects', 'gt', $value))); $this->w->req->end(); return $this; }
    public function countProjectsLte(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w->navCount('projects', 'lte', $value))); $this->w->req->end(); return $this; }
    public function countProjectsLt(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w->navCount('projects', 'lt', $value))); $this->w->req->end(); return $this; }
    public function countProjectsNotEq(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w->navCount('projects', 'not_eq', $value))); $this->w->req->end(); return $this; }
...
/** Query over account: Account::query() → using($db) → typed chain → terminal(). */
final class Account extends Q implements AccountInterface
{
    public const ENTITY = 'account';

    private function __construct() { parent::__construct('account'); }
    public static function query(): static { return new static(); }

    // ---- WHERE ----
    public function or(?\Closure $fn = null): static { if ($fn === null) { $this->orConn(); return $this; } $this->orConn(); $fn(new AccountWhere($this->w()->group())); $this->req->end(); return $this; }
    public function and(?\Closure $fn = null): static { if ($fn === null) { return $this; } $fn(new AccountWhere($this->w()->group())); $this->req->end(); return $this; }
    public function expr(string $frag, array $binds = []): static { $this->w()->expr($frag, $binds); return $this; }
    public function projects(\Closure $fn): static { $fn(new ProjectWhere($this->w()->nav('projects'))); $this->req->end(); return $this; }
    public function hasProjects(\Closure $fn): static { $fn(new ProjectWhere($this->w()->navMode('projects', 'exists'))); $this->req->end(); return $this; }
    public function notHasProjects(\Closure $fn): static { $fn(new ProjectWhere($this->w()->navMode('projects', 'not_exists'))); $this->req->end(); return $this; }
    public function countProjectsEq(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w()->navCount('projects', 'eq', $value))); $this->req->end(); return $this; }
    public function countProjectsGte(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w()->navCount('projects', 'gte', $value))); $this->req->end(); return $this; }
    public function countProjectsGt(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w()->navCount('projects', 'gt', $value))); $this->req->end(); return $this; }
    public function countProjectsLte(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w()->navCount('projects', 'lte', $value))); $this->req->end(); return $this; }
    public function countProjectsLt(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w()->navCount('projects', 'lt', $value))); $this->req->end(); return $this; }
    public function countProjectsNotEq(int $value, \Closure $fn): static { $fn(new ProjectWhere($this->w()->navCount('projects', 'not_eq', $value))); $this->req->end(); return $this; }
    ...
    public function on(\Closure $fn): static { $fn(new AccountWhere($this->onW())); return $this; }
    public function where(\Closure $fn): static { $fn(new AccountWhere($this->w())); return $this; }

    public function joinSeqWithSeq(Project $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'join expects one child query'); } $this->attachJoin('projects', 'inner', $child); return $this; }
    public function leftJoinSeqWithSeq(Project $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'leftJoin expects one child query'); } $this->attachJoin('projects', 'left', $child); return $this; }
    public function relationsSeqWithSeq(Project $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'relations expects one child query'); } $this->attachRelation('projects', $child); return $this; }
    ...
    public function having(\Closure $fn): static { $fn(new AccountWhere($this->havingW())); return $this; }
    ...
    // ---- terminals ----
    public function get(): AccountRow
    {
        $row = $this->getOrNull();
    ...
    public function getOrNull(): ?AccountRow
    {
        $this->terminalArity(func_num_args());
        $db = $this->terminalDb();
        $rows = $this->runQuery($db, 'one');
        return $rows->data === [] ? null : AccountRow::fromRow($rows->data[0], $rows->asm, $rows);
    }
    ...
    public function gets(): Collection
    {
        $this->terminalArity(func_num_args());
        $db = $this->terminalDb();
        return Collection::fromRows($this->runQuery($db, 'all'), AccountRow::class, $this->keyFn);
    }
    ...
    public function stream(callable $visit): StreamResult
    {
        ...
        return $db->streamPlan($plan, $this->req->params, static function (array $values, Rows $rows) use ($visit): bool {
            return (bool) $visit(AccountRow::fromRow($values, $rows->asm, $rows));
        });
    }
    ...
    public function paginate(int $page, int $per): Page
    {
        ...
        return new Page(Collection::fromRows($rows, AccountRow::class, $this->keyFn), $total, intdiv($total + $per - 1, $per), max(1, $page), $per);
    }
    ...
    public function getsAfter(string $cursor, int $per): KeysetPage
    {
        ...
        return KeysetPage::fromRows($rows, Collection::fromRows($rows, AccountRow::class, $this->keyFn), $this->req->ir['order'] ?? []);
    }
    ...
    public function insert(): ?AccountRow
    {
        $this->terminalArity(func_num_args());
        $db = $this->terminalDb();
        $id = $this->runInsert($db);
        return Account::query()->using($db)->seqEq((int) $id)->get();
    }

    /** UPDATE when every primary-key column was assigned; otherwise INSERT. */
    public function save(): ?AccountRow
    {
        $this->terminalArity(func_num_args());
        $db = $this->terminalDb();
        [$updated, $keys] = $this->runSave($db, ['seq']);
        if (!$updated) { $keys = [(int) $keys[0]]; }
        return Account::query()->using($db)->seqEq($keys[0])->get();
    }
    ...
    /** @param list<Account> $rows */
    private function batchWrite(array $rows, string $kind, ?BatchOptions $options): BatchResult
    {
        $db = $this->terminalDb(); $requests = [];
        foreach ($rows as $row) {
            if (!$row instanceof Account) throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'batch row has the wrong type');
    ...
    public function getBySeq(int $value): ?AccountRow
    {
        $this->terminalArity(func_num_args(), 1);
