...
final readonly class ServiceModuleKey
{
...
/** Where builder for service_module: typed predicates, or(), and(fn), and relation navigation. */
...
/** Query over service_module: ServiceModule::query() → using($db) → typed chain → terminal(). */
final class ServiceModule extends Q implements ServiceModuleInterface
{
    ...
    // ---- WHERE ----
    public function or(?\Closure $fn = null): static { if ($fn === null) { $this->orConn(); return $this; } $this->orConn(); $fn(new ServiceModuleWhere($this->w()->group())); $this->req->end(); return $this; }
    public function and(?\Closure $fn = null): static { if ($fn === null) { return $this; } $fn(new ServiceModuleWhere($this->w()->group())); $this->req->end(); return $this; }
    public function expr(string $frag, array $binds = []): static { $this->w()->expr($frag, $binds); return $this; }
    ...
    public function joinSeqWithServiceModuleSeq(Battle $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'join expects one child query'); } $this->attachJoin('battles', 'inner', $child); return $this; }
    public function leftJoinSeqWithServiceModuleSeq(Battle $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'leftJoin expects one child query'); } $this->attachJoin('battles', 'left', $child); return $this; }
    public function relationsSeqWithServiceModuleSeq(Battle $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'relations expects one child query'); } $this->attachRelation('battles', $child); return $this; }
    ...
    public function getOrNull(): ?ServiceModuleRow
    {
    ...
    public function getsAfter(string $cursor, int $per): KeysetPage
    {
        ...
        return KeysetPage::fromRows($rows, Collection::fromRows($rows, ServiceModuleRow::class, $this->keyFn), $this->req->ir['order'] ?? []);
    }
    ...
    public function insert(): ?ServiceModuleRow
    {
        ...
        return ServiceModule::query()->using($db)->seqEq((int) $id)->get();
    }
    ...
    public function save(): ?ServiceModuleRow
    {
        ...
        return ServiceModule::query()->using($db)->seqEq($keys[0])->get();
    }
    ...
    /** @param list<ServiceModule> $rows */
    private function batchWrite(array $rows, string $kind, ?BatchOptions $options): BatchResult
    {
        $db = $this->terminalDb(); $requests = [];
        foreach ($rows as $row) {
            if (!$row instanceof ServiceModule) throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'batch row has the wrong type');
