...
final class ServiceRow extends Row implements ServiceRowInterface
{
    ...
    {
        return ['battles' => ['kind' => 'many', 'target' => 'battle', 'left' => 'seq', 'right' => 'service_seq'], 'members' => ['kind' => 'many', 'target' => 'service_member', 'left' => 'seq', 'right' => 'service_seq'], 'modules' => ['kind' => 'many', 'target' => 'service_module', 'left' => 'seq', 'right' => 'service_seq']];
    }
    ...
    public function getMembers(): Collection { return $this->relation('members') ?? new Collection(); }
    public function getModules(): Collection { return $this->relation('modules') ?? new Collection(); }
}

/** Column references for column-to-column predicates (ServiceCols::seq()); ->at('service') points into a joined entity. */
final class ServiceCols
{
...
final class ServiceWhere
{
    ...
    public function countModulesEq(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w->navCount('modules', 'eq', $value))); $this->w->req->end(); return $this; }
    public function countModulesGte(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w->navCount('modules', 'gte', $value))); $this->w->req->end(); return $this; }
    public function countModulesGt(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w->navCount('modules', 'gt', $value))); $this->w->req->end(); return $this; }
    public function countModulesLte(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w->navCount('modules', 'lte', $value))); $this->w->req->end(); return $this; }
    public function countModulesLt(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w->navCount('modules', 'lt', $value))); $this->w->req->end(); return $this; }
    public function countModulesNotEq(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w->navCount('modules', 'not_eq', $value))); $this->w->req->end(); return $this; }
...
final class Service extends Q implements ServiceInterface
{
    ...
    // ---- WHERE ----
    public function or(?\Closure $fn = null): static { if ($fn === null) { $this->orConn(); return $this; } $this->orConn(); $fn(new ServiceWhere($this->w()->group())); $this->req->end(); return $this; }
    public function and(?\Closure $fn = null): static { if ($fn === null) { return $this; } $fn(new ServiceWhere($this->w()->group())); $this->req->end(); return $this; }
    public function expr(string $frag, array $binds = []): static { $this->w()->expr($frag, $binds); return $this; }
    ...
    public function countModulesEq(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w()->navCount('modules', 'eq', $value))); $this->req->end(); return $this; }
    public function countModulesGte(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w()->navCount('modules', 'gte', $value))); $this->req->end(); return $this; }
    public function countModulesGt(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w()->navCount('modules', 'gt', $value))); $this->req->end(); return $this; }
    public function countModulesLte(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w()->navCount('modules', 'lte', $value))); $this->req->end(); return $this; }
    public function countModulesLt(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w()->navCount('modules', 'lt', $value))); $this->req->end(); return $this; }
    public function countModulesNotEq(int $value, \Closure $fn): static { $fn(new ServiceModuleWhere($this->w()->navCount('modules', 'not_eq', $value))); $this->req->end(); return $this; }
    ...
    public function joinSeqWithServiceSeqToBattle(Battle $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'join expects one child query'); } $this->attachJoin('battles', 'inner', $child); return $this; }
    public function leftJoinSeqWithServiceSeqToBattle(Battle $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'leftJoin expects one child query'); } $this->attachJoin('battles', 'left', $child); return $this; }
    public function relationsSeqWithServiceSeqToBattle(Battle $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'relations expects one child query'); } $this->attachRelation('battles', $child); return $this; }
    public function joinSeqWithServiceSeqToServiceMember(ServiceMember $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'join expects one child query'); } $this->attachJoin('members', 'inner', $child); return $this; }
    public function leftJoinSeqWithServiceSeqToServiceMember(ServiceMember $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'leftJoin expects one child query'); } $this->attachJoin('members', 'left', $child); return $this; }
    public function relationsSeqWithServiceSeqToServiceMember(ServiceMember $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'relations expects one child query'); } $this->attachRelation('members', $child); return $this; }
    public function joinSeqWithServiceSeqToServiceModule(ServiceModule $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'join expects one child query'); } $this->attachJoin('modules', 'inner', $child); return $this; }
    public function leftJoinSeqWithServiceSeqToServiceModule(ServiceModule $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'leftJoin expects one child query'); } $this->attachJoin('modules', 'left', $child); return $this; }
    public function relationsSeqWithServiceSeqToServiceModule(ServiceModule $child): static { if (func_num_args() !== 1) { throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'relations expects one child query'); } $this->attachRelation('modules', $child); return $this; }
    ...
    public function having(\Closure $fn): static { $fn(new ServiceWhere($this->havingW())); return $this; }
    ...
    public function getOrNull(): ?ServiceRow
    {
    ...
    public function getsAfter(string $cursor, int $per): KeysetPage
    {
        ...
        return KeysetPage::fromRows($rows, Collection::fromRows($rows, ServiceRow::class, $this->keyFn), $this->req->ir['order'] ?? []);
    }
    ...
    public function insert(): ?ServiceRow
    {
        $this->terminalArity(func_num_args());
        $db = $this->terminalDb();
    ...
    /** @param list<Service> $rows */
    private function batchWrite(array $rows, string $kind, ?BatchOptions $options): BatchResult
    {
        $db = $this->terminalDb(); $requests = [];
        foreach ($rows as $row) {
            if (!$row instanceof Service) throw new \Orm\OrmException(\Orm\Code::IR_INVALID, 'batch row has the wrong type');
