...
final class ServiceMember extends Q
{
    ...
    public function one(Db $db): ?ServiceMemberRow
    {
        ...
        return $rows === [] ? null : ServiceMemberRow::fromRow($rows[0], $asm);
    }

    public function all(Db $db): Collection
    {
        [$rows, $asm] = $this->runQuery($db, 'all');
        return Collection::fromRows($rows, $asm, ServiceMemberRow::class);
    }
    ...
    public function paginate(Db $db, int $page, int $per): Page
    {
        ...
        return new Page(Collection::fromRows($rows, $asm, ServiceMemberRow::class), $total, intdiv($total + $per - 1, $per), max(1, $page), $per);
    }
