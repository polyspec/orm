    ...
    private static function group(array $g, string $path): Group
    {
        ...
        foreach ($g['items'] ?? [] as $i => $value) {
            if (isset($value['pred'])) {
                ...
                $n = $value['nav'];
                $navigation = ['connector' => (string) ($n['conn'] ?? ''), 'relation' => (string) $n['rel'], 'group' => self::group($n['group'], "$path.items[$i].navigation.group"), 'mode' => (string) ($n['mode'] ?? ''), 'count_operator' => (string) ($n['count_op'] ?? '')];
                if (isset($n['p'])) $navigation['count_parameter'] = (int) $n['p'];
                $items[] = new Item(['navigation' => new Navigation($navigation)]);
