...
final class SchemaTriggers
{
    ...
    private static function postgresAudit(array $m, array $e, array $a, string $markers): array
    {
        ...
        if ($a['mode'] === 'changes') {
            ...
            $b .= "  END IF;\n";
            // The service value keeps its column type, so the change table's service column can have
            // any type the audited column converts to.
            $service = $a['service'] === '' ? 'NULL' : "CASE TG_OP WHEN 'DELETE' THEN OLD." . $q($a['service']) . ' ELSE NEW.' . $q($a['service']) . ' END';
            $keys = [];
            foreach ($e['pk'] as $k) {
                $keys[] = self::literal($k);
            ...
            $b .= '  INSERT INTO ' . $q($l['change']['table']) . ' (' . self::changeColumns($l, $q) . ') VALUES (audit_operation_seq, TG_OP, ' . $service . ', ' . self::literal($e['table']) . ", audit_key, audit_old, audit_new);\n";
        }
        $b .= "  RETURN NULL;\nEND\n$$;";
        $table = $q($e['table']);
        return [
