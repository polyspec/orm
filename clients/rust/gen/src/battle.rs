    ...
    pub(crate) fn from_row(vals: &mut [Val], a: &orm::plan::Assemble) -> Self {
        ...
        for ch in &a.children {
            ...
            match ch.rel.as_str() {
                ...
                "service_member" => if db::join_present(vals, &ch.assemble) { r.service_member_ = Some(Box::new(super::service_member::ServiceMemberRow::from_row(vals, &ch.assemble))); },
                "service_module" => if db::join_present(vals, &ch.assemble) { r.service_module_ = Some(Box::new(super::service_module::ServiceModuleRow::from_row(vals, &ch.assemble))); },
                "user" => if db::join_present(vals, &ch.assemble) { r.user_ = Some(Box::new(super::user::UserRow::from_row(vals, &ch.assemble))); },
                _ => {}
            }
        }
        r
