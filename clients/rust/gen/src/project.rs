...
impl Project {
    ...
    // ---- terminals ----
    pub async fn get(&mut self) -> Result<Option<ProjectRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn insert(&mut self) -> Result<Option<ProjectRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
        ...
        super::project::query().using(ex).seq_eq(id as i64).get().await
    }
