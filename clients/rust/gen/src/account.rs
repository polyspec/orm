...
impl Account {
    ...
    // ---- terminals ----
    pub async fn get(&mut self) -> Result<Option<AccountRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn insert(&mut self) -> Result<Option<AccountRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
        ...
        super::account::query().using(ex).seq_eq(id as i64).get().await
    }
