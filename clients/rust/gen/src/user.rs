...
impl User {
    ...
    // ---- terminals ----
    pub async fn get(&mut self) -> Result<UserRow> { let row = self.get_or_none().await?; row.ok_or(orm::Error::NoRows) }

    pub async fn get_or_none(&mut self) -> Result<Option<UserRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
