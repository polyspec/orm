...
impl ServiceModule {
    ...
    pub async fn one(mut self, ex: &impl Exec) -> Result<Option<ServiceModuleRow>> {
        ...
        Ok(rows.data.first().map(|v| ServiceModuleRow::from_row(v, &rows.assemble)))
    }
...
fn collect(rows: &db::Rows) -> Collection<ServiceModuleRow> {
    let mut c = Collection::with_capacity(rows.data.len());
    for v in &rows.data {
        c.put(Key::of(&v[0]), ServiceModuleRow::from_row(v, &rows.assemble));
    }
    c
}
