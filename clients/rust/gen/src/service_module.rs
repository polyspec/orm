...
impl orm::collection::RowExport for ServiceModuleRow {
...
impl ServiceModule {
    ...
    pub fn relation(mut self, child: impl AsRef<Q>) -> Self {
        ...
        match (c.entity(), c.link_left.as_str(), c.link_right.as_str()) {
            ...
            _ => panic!("no one-to-one relation from service_module"),
        }
    }
    pub fn relations(mut self, child: impl AsRef<Q>) -> Self {
        let c = child.as_ref();
        match (c.entity(), c.link_left.as_str(), c.link_right.as_str()) {
            ("battle", "seq", "service_module_seq") => { self.q.relation("battles", c); self },
            ("battle", "", "") => { self.q.relation("battles", c); self },
            _ => panic!("no one-to-many relation from service_module"),
        }
    }
    ...
    pub fn match_service_module_seq_with_seq(mut self) -> Self { self.q.set_link("service_module_seq", "seq"); self }
    pub fn on_service_module_seq_with_seq(mut self) -> Self { self.q.set_link("service_module_seq", "seq"); self }
    ...
    // ---- terminals ----
    pub async fn get(&mut self) -> Result<ServiceModuleRow> { let row = self.get_or_none().await?; row.ok_or(orm::Error::NoRows) }

    pub async fn get_or_none(&mut self) -> Result<Option<ServiceModuleRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn gets(&mut self) -> Result<Collection<ServiceModuleRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
        let mut rows = db::select(ex, &mut self.q.req, "all").await?;
        collect(&mut rows, self.key_fn.as_deref())
    }

    pub async fn gets_after(&mut self, cursor: &str, per: u32) -> Result<orm::KeysetPage<ServiceModuleRow>> {
    ...
    pub async fn gets_before(&mut self, cursor: &str, per: u32) -> Result<orm::KeysetPage<ServiceModuleRow>> {
    ...
    pub async fn stream(&mut self, visit: impl FnMut(ServiceModuleRow) -> bool) -> Result<db::StreamResult> {
        let mut visit = visit;
        let binding = self.binding.clone();
        let ex = binding.resolve()?;
        db::stream(ex, &mut self.q.req, |mut src, rows| {
            let row = ServiceModuleRow::from_row(&mut src, &rows.assemble, rows)?;
            Ok(visit(row))
        }).await
    }
    ...
    pub async fn gets_by_seq(&mut self, v: i64) -> Result<Collection<ServiceModuleRow>> {
        self.q.w().pred("seq", "eq", v);
        self.gets().await
    }

    /// Applies service_seq = value and runs the collection terminal.
    pub async fn gets_by_service_seq(&mut self, v: i64) -> Result<Collection<ServiceModuleRow>> {
        self.q.w().pred("service_seq", "eq", v);
        self.gets().await
    }

    /// Applies name = value and runs the collection terminal.
    pub async fn gets_by_name(&mut self, v: impl Into<String>) -> Result<Collection<ServiceModuleRow>> {
    ...
    pub async fn gets_count(&mut self) -> Result<Collection<ServiceModuleRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn paginate(&mut self, page: u32, per: u32) -> Result<Page<ServiceModuleRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn batch_insert(&self, rows: Vec<ServiceModule>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "insert", options).await }
    pub async fn batch_upsert(&self, rows: Vec<ServiceModule>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "insert", options).await }
    pub async fn batch_update(&self, rows: Vec<ServiceModule>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "update", options).await }
    pub async fn batch_delete(&self, rows: Vec<ServiceModule>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "delete", options).await }
    async fn batch_write(&self, rows: Vec<ServiceModule>, kind: &str, options: orm::BatchOptions) -> Result<orm::BatchResult> {
        let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn get_by_seq(&mut self, v: i64) -> Result<Option<ServiceModuleRow>> {
        self.q.w().pred("seq", "eq", v);
...
impl AsRef<Q> for ServiceModule { fn as_ref(&self) -> &Q { &self.q } }
