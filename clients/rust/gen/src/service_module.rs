...
impl ServiceModule {
    ...
    // ---- terminals ----
    pub async fn get(&mut self) -> Result<Option<ServiceModuleRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn insert(&mut self) -> Result<Option<ServiceModuleRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
        ...
        super::service_module::query().using(ex).seq_eq(id as i64).get().await
    }
