...
impl<'a> ServiceMemberWhere<'a> {
    ...
    pub fn and_group(mut self, f: impl FnOnce(ServiceMemberWhere<'_>) -> ServiceMemberWhere<'_>) -> Self { self.w.and_with(|w| { f(ServiceMemberWhere { w }); }); self }
...
impl ServiceMember {
    ...
    pub fn and_group(mut self, f: impl FnOnce(ServiceMemberWhere<'_>) -> ServiceMemberWhere<'_>) -> Self { self.q.w().and_with(|w| { f(ServiceMemberWhere { w }); }); self }
    ...
    pub fn relation(mut self, child: impl AsRef<Q>) -> Self {
        ...
        match (c.entity(), c.link_left.as_str(), c.link_right.as_str()) {
            ...
            _ => panic!("no one-to-one relation from service_member"),
        }
    }
    pub fn relations(mut self, child: impl AsRef<Q>) -> Self {
        let c = child.as_ref();
        match (c.entity(), c.link_left.as_str(), c.link_right.as_str()) {
            ("battle", "seq", "service_member_seq") => { self.q.relation("battles", c); self },
            ("battle", "", "") => { self.q.relation("battles", c); self },
            _ => panic!("no one-to-many relation from service_member"),
        }
    }
    ...
    fn join_target(mut self, child: impl AsRef<Q>, _kind: &str) -> Self {
        let c = child.as_ref();
        match (c.entity(), c.link_left.as_str(), c.link_right.as_str()) {
            ("battle", "seq", "service_member_seq") => { self.q.join("battles", _kind, c); self },
    ...
    pub fn join_seq_with_service_member_seq(mut self, child: impl AsRef<super::battle::Battle>) -> Self { self.q.join("battles", "inner", &child.as_ref().q); self }
    pub fn left_join_seq_with_service_member_seq(mut self, child: impl AsRef<super::battle::Battle>) -> Self { self.q.join("battles", "left", &child.as_ref().q); self }
    pub fn relations_seq_with_service_member_seq(mut self, child: impl AsRef<super::battle::Battle>) -> Self { self.q.relation("battles", &child.as_ref().q); self }
    ...
    pub fn match_service_member_seq_with_seq(mut self) -> Self { self.q.set_link("service_member_seq", "seq"); self }
    pub fn on_service_member_seq_with_seq(mut self) -> Self { self.q.set_link("service_member_seq", "seq"); self }
    ...
    pub async fn gets(&mut self) -> Result<Collection<ServiceMemberRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
        let mut rows = db::select(ex, &mut self.q.req, "all").await?;
        collect(&mut rows, self.key_fn.as_deref())
    }

    pub async fn gets_after(&mut self, cursor: &str, per: u32) -> Result<orm::KeysetPage<ServiceMemberRow>> {
    ...
    pub async fn gets_before(&mut self, cursor: &str, per: u32) -> Result<orm::KeysetPage<ServiceMemberRow>> {
    ...
    pub async fn stream(&mut self, visit: impl FnMut(ServiceMemberRow) -> bool) -> Result<db::StreamResult> {
        let mut visit = visit;
        let binding = self.binding.clone();
        let ex = binding.resolve()?;
        db::stream(ex, &mut self.q.req, |mut src, rows| {
            let row = ServiceMemberRow::from_row(&mut src, &rows.assemble, rows)?;
            Ok(visit(row))
        }).await
    }
    ...
    pub async fn gets_by_seq(&mut self, v: i64) -> Result<Collection<ServiceMemberRow>> {
        self.q.w().pred("seq", "eq", v);
        self.gets().await
    }

    /// Applies service_seq = value and runs the collection terminal.
    pub async fn gets_by_service_seq(&mut self, v: i64) -> Result<Collection<ServiceMemberRow>> {
        self.q.w().pred("service_seq", "eq", v);
        self.gets().await
    }

    /// Applies user_seq = value and runs the collection terminal.
    pub async fn gets_by_user_seq(&mut self, v: i64) -> Result<Collection<ServiceMemberRow>> {
    ...
    pub async fn gets_count(&mut self) -> Result<Collection<ServiceMemberRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn paginate(&mut self, page: u32, per: u32) -> Result<Page<ServiceMemberRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn save(&mut self) -> Result<Option<ServiceMemberRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn batch_insert(&self, rows: Vec<ServiceMember>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "insert", options).await }
    pub async fn batch_upsert(&self, rows: Vec<ServiceMember>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "insert", options).await }
    pub async fn batch_update(&self, rows: Vec<ServiceMember>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "update", options).await }
    pub async fn batch_delete(&self, rows: Vec<ServiceMember>, options: orm::BatchOptions) -> Result<orm::BatchResult> { self.batch_write(rows, "delete", options).await }
    async fn batch_write(&self, rows: Vec<ServiceMember>, kind: &str, options: orm::BatchOptions) -> Result<orm::BatchResult> {
        let binding = self.binding.clone(); let ex = binding.resolve()?;
...
impl AsRef<Q> for ServiceMember { fn as_ref(&self) -> &Q { &self.q } }
...
fn collect(rows: &mut db::Rows, key_fn: Option<&(dyn Fn(&ServiceMemberRow) -> Key + Send + Sync)>) -> Result<Collection<ServiceMemberRow>> {
    ...
    for mut src in cells {
        ...
        let r = ServiceMemberRow::from_row(&mut src, &rows.assemble, rows)?;
