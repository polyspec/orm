...
impl ServiceMember {
    ...
    // ---- terminals ----
    pub async fn one(&mut self) -> Result<Option<ServiceMemberRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn all(&mut self) -> Result<Collection<ServiceMemberRow>> { let binding = self.binding.clone(); let ex = binding.resolve()?;
    ...
    pub async fn get(&mut self) -> Result<Option<ServiceMemberRow>> {
        self.one().await
    }

    /// Preferred collection terminal; all() remains available for compatibility.
    pub async fn gets(&mut self) -> Result<Collection<ServiceMemberRow>> {
        self.all().await
    }
    ...
    pub async fn one_by_seq(&mut self, v: i64) -> Result<Option<ServiceMemberRow>> {
        self.q.w().pred("seq", "eq", v);
        self.one().await
    }
