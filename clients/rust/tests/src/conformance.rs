...
async fn main() {
    ...
    let db = Db::connect_with_compiler(opts, 4, engine, compiler, Config { aes_key: "bench-salt".into(), aes_version: 1, aes_keys: [(1, "bench-salt".into())].into_iter().collect(), on_query: Some(on_query) }).await.expect("connect");
    ...
    run!("interface_invalid_page", async { battle::query().using(&db).paginate(1,0).await?;Ok(Value::Null) }.await);
    ...
    run!("interface_dirty_retry", async {
        ...
        let err=db.transaction(|tx|{let observed=observed.clone();let log=log.clone();let mask=mask.clone();async move {
            ...
            battle::query().using(&tx).seq(6).set_updated_ts(chrono::NaiveDate::from_ymd_opt(2001,1,1).unwrap().and_hms_opt(0,0,0).unwrap()).update().await?;
    ...
    run!("bound_transaction_rollback", async {
        ...
        let err = db.transaction(|tx| {
            ...
            let db = &db;
            async move {
                let mut s = service::query().using(&tx).set_name("conf-bind").insert().await?.unwrap();
                ...
                let changed = service_member::query().using(&tx).user_seq(2).get_count_by_service_seq(s.seq).await?;
        ...
        let members_left = service_member::query().using(&db).get_count_by_service_seq(seq).await?;
    ...
    run!("pk_one", async { Ok(row(battle::query().seq(42).using(&db).get().await?.as_ref())) }.await);
    run!("pk_one_by", async { Ok(row(battle::query().using(&db).get_by_seq(42).await?.as_ref())) }.await);
    run!("pk_missing", async { Ok(row(battle::query().seq(0).using(&db).get().await?.as_ref())) }.await);
    run!("select_lazy", async {
        let b = battle::query().select_description().seq(42).using(&db).get().await?.unwrap();
        Ok(json!({"seq": b.seq, "description_prefix": &b.description.as_deref().unwrap()[..7]}))
    }.await);
    run!("list_order_limit", async {
        Ok(keyed(&battle::query().service_seq(7).is_close(false).order_by_seq_desc().limit(0, 5).using(&db).gets().await?))
    }.await);
    run!("in_keyed", async { Ok(keys(&battle::query().seq_in(vec![306, 6, 106]).order_by_seq_asc().using(&db).gets().await?)) }.await);
    run!("group_or", async {
        Ok(keyed(&battle::query()
            .service_seq(7)
            .is_close(false)
    ...
    run!("contains_escape", async { Ok(json!(battle::query().name_contains("%").using(&db).get_count().await?)) }.await);
    run!("empty_in_error", async { Ok(json!(battle::query().seq_in(vec![]).using(&db).get_count().await?)) }.await);
    ...
    run!("write_cycle", async {
        ...
        again.delete().await?;
        let left = battle::query().seq(created.seq).using(&db).get_count().await?;
        Ok(json!({
    ...
    run!("select_expr", async {
        let b = battle::query().select_expr("tag", "CONCAT(`name`, '!')").seq(42).using(&db).get().await?.unwrap();
    ...
    run!("upsert", async {
        let (a, b) = db.transaction(|tx| async move {
            let a = fks(battle::query().set_uuid(Some("conf-upsert")).set_name("u1").set_read_count(1)).using(&tx).insert().await?.unwrap();
            let b = fks(battle::query().set_uuid(Some("conf-upsert")).set_name("u2").set_read_count(1))
                .on_duplicate_set_name("u2").on_duplicate_plus_read_count(5)
                .using(&tx).insert().await?.unwrap();
            b.delete().await?;
            Ok((a, b))
        }).await?;
    ...
    run!("upsert_set_all", async {
        let (a, b) = db.transaction(|tx| async move {
            let a = fks(battle::query().set_uuid(Some("conf-upsert")).set_name("u1").set_read_count(1)).using(&tx).insert().await?.unwrap();
            let b = fks(battle::query().set_uuid(Some("conf-upsert")).set_name("u3").set_read_count(9)).on_duplicate_set_all().using(&tx).insert().await?.unwrap();
            b.delete().await?;
            Ok((a, b))
        }).await?;
        mask_created(&mask, &log, Mask { seqs: vec![a.seq], ts: Some(a.updated_ts) });
    ...
    run!("save_branch", async {
        let r = db.transaction(|tx| async move { fks(battle::query().set_name("conf-save")).using(&tx).save().await }).await?.unwrap();
        mask_created(&mask, &log, Mask { seqs: vec![r.seq], ts: Some(r.updated_ts) });
        let after = battle::query().set_seq(r.seq).set_name("conf-save-2").using(&db).save().await?.unwrap();
        after.delete().await?;
    ...
    run!("group_count_having", async {
        Ok(json!(battle::query().service_seq(7).group_by_user_seq().having(|w| w.expr("COUNT(*) > ?", vec![1.into()])).using(&db).get_count().await?))
    }.await);
