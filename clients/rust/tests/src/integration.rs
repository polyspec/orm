...
async fn main() {
    ...
    // ---- reads ----
    let b = Battle::new().bind(&db).one_by_seq(42).await.expect("one").expect("row 42");
    ...
    let b2 = Battle::new().select_description().seq_eq(42).bind(&db).one().await.unwrap().unwrap();
    ...
    check!(fails, Battle::new().service_seq_eq(7).bind(&db).count().await.unwrap() == 1000, "count");
    check!(fails, Battle::new().service_seq_eq(7).bind(&db).sum_like_count().await.unwrap() > 0.0, "sum");
    ...
    let page = Battle::new().service_seq_eq(7).order_by_seq_asc().bind(&db).paginate(2, 10).await.expect("paginate");
    check!(fails, page.total == 1000 && page.pages == 100 && page.items.len() == 10 && page.items.first().map(|r| r.seq) == Some(1006), "paginate");
    check!(fails, Battle::new().name_contains("%").bind(&db).count().await.unwrap() == 0, "contains escapes %");

    let j = Battle::new().join_service(Service::new().where_(|w| w.seq_eq(7))).seq_eq(6).bind(&db).one().await.unwrap().unwrap();
    ...
    check!(fails, Battle::new().service_seq_eq(7).bind(&db).count_distinct_user_seq().await.unwrap() == 50, "count_distinct");
    check!(fails, Battle::new().service_seq_eq(7).bind(&db).min_seq().await.unwrap() == Some(6), "min typed (i64)");
    check!(fails, Battle::new().service_seq_eq(7).bind(&db).max_seq().await.unwrap() == Some(99906), "max typed (i64)");
    check!(fails, Battle::new().seq_eq(0).bind(&db).max_seq().await.unwrap().is_none(), "max over no rows is None");
    check!(fails, Battle::new().seq_eq(0).bind(&db).min_name().await.unwrap().is_none(), "min String over no rows is None");
    let mx = Battle::new().service_seq_eq(7).bind(&db).max_start_dt().await.unwrap();
    check!(fails, mx.map(|t| t.date().year() >= 2020).unwrap_or(false), "max typed (NaiveDateTime)");
    check!(fails, Battle::new().service_seq_eq(7).bind(&db).max_price().await.unwrap().is_none(), "max over a NULL-only column is None");
    check!(fails, Battle::new().service_seq_eq(7).group_by_user_seq().bind(&db).count().await.unwrap() == 50, "count + group_by = number of groups");
    check!(fails, Battle::new().service_seq_eq(7).group_by_user_seq().having(|w| w.expr("COUNT(*) > ?", vec![1.into()])).bind(&db).count().await.unwrap() == 50, "having on group count");
    check!(fails, Battle::new().service_seq_eq(7).group_by_user_seq().having(|w| w.expr("COUNT(*) > ?", vec![1000.into()])).bind(&db).count().await.unwrap() == 0, "having filters every group");
    // row select keeps HAVING (grouped by the PK so only_full_group_by holds): every group is one row
    let grouped = Battle::new().service_seq_eq(7).group_by_seq().having(|w| w.expr("COUNT(*) > ?", vec![0.into()])).order_by_seq_asc().limit(0, 2).bind(&db).all().await.expect("having rows");
    check!(fails, grouped.len() == 2 && grouped.first().map(|b| b.seq) == Some(6), "having on row select");
    check!(fails, Battle::new().service_seq_eq(7).group_by_seq().having(|w| w.expr("COUNT(*) > ?", vec![1.into()])).limit(0, 2).bind(&db).all().await.unwrap().is_empty(), "having on row select filters every group");
    match Battle::new().service_seq_eq(7).having(|w| w.expr("COUNT(*) > ?", vec![1.into()])).bind(&db).count().await {
        Err(e) if e.code() == "IR_INVALID" => {}
        other => { fails += 1; eprintln!("FAIL: having without group_by not rejected: {:?}", other.err()); }
    }
    let visible = Battle::new().visible().service_seq_eq(7).bind(&db).count().await.expect("visible");
    check!(fails, visible == Battle::new().service_seq_eq(7).is_close_eq(false).is_display_eq(true).bind(&db).count().await.unwrap() && visible > 0, "predicate visible() = is_close 0 AND is_display 1");
    check!(fails, Battle::new().started_after("2026-01-01 00:00:00").service_seq_eq(7).bind(&db).count().await.unwrap() == 1000, "predicate started_after(v) on the query");
    check!(fails, Battle::new().service_seq_eq(7).and(|w| w.visible().or().started_after("2999-01-01 00:00:00")).bind(&db).count().await.unwrap() == visible, "predicates on the Where builder honour or()");
    let all_visible = Battle::new().visible().bind(&db).count().await.unwrap();
    check!(fails, all_visible > visible && Battle::new().seq_eq(0).or().visible().bind(&db).count().await.unwrap() == all_visible, "query predicate honours a pending or()");
    let rows = Battle::new().raw("SELECT COUNT(*) AS n, MAX(seq) AS m, MIN(start_dt) AS d FROM {table} WHERE service_seq = ? AND is_close = ?", vec![7.into(), false.into()]).bind(&db).raw_all().await.expect("raw_all");
    ...
    check!(fails, Battle::new().raw("SELECT seq FROM {table} WHERE seq = ?", vec![0.into()]).bind(&db).raw_all().await.unwrap().is_empty(), "raw_all: no rows = empty list");
    match Battle::new().raw("SELECT seq FROM {table} WHERE seq = ?", vec![]).bind(&db).raw_all().await {
        Err(e) if e.code() == "IR_INVALID" => {}
    ...
    let rows = Battle::new()
        ...
        .bind(&db).all().await.expect("relations");
    ...
    let rows = Battle::new()
        ...
        .bind(&db).all().await.expect("if_parent");
    ...
    let n0 = statements.load(Ordering::Relaxed);
    let none = Battle::new().seq_eq(0).relation_user(User::new()).bind(&db).all().await.expect("empty");
    check!(fails, none.is_empty() && statements.load(Ordering::Relaxed) - n0 == 1, "no parents → relation step skipped");
    let one = Battle::new().seq_eq(42).relation_service(Service::new().relations_members(ServiceMember::new().limit_per_parent(1))).bind(&db).one().await.expect("one").expect("row 42");
    ...
    let m = ServiceMember::new().service_seq_eq(7).order_by_seq_asc().limit(0, 2).relation_user(User::new().flatten()).bind(&db).all().await.expect("flatten");
    check!(fails, m.first().and_then(|m| m.user()).map(|u| u.name.starts_with("user-")) == Some(true), "flatten");
    let page = Battle::new().service_seq_eq(7).order_by_seq_asc().relation_user(User::new()).bind(&db).paginate(1, 4).await.expect("paginate");
    ...
    let mut cancelled = Box::pin(db.transaction(|tx| {
        ...
        async move {
            let r = Service::new().bind(&tx).set_name("interface-cancel").insert().await?;
    ...
    let remaining = Service::new().bind(&db).get_count_by_name("interface-cancel").await.unwrap();
    ...
    c.bind(&db).update_optimistic().await.expect("update");
    let again = Battle::new().bind(&db).one_by_seq(created.seq).await.unwrap().unwrap();
    check!(fails, again.name == "rust-write-2" && again.like_count == 5, "dirty update");
    c.set_name("stale");
    match c.bind(&db).update_optimistic().await {
    ...
    check!(fails, Battle::new().seq_eq(created.seq).bind(&db).count().await.unwrap() == 0, "delete");

    match Battle::new().seq_in(vec![]).bind(&db).count().await {
    ...
    let draft = |name: &str| Battle::new()
        ...
        .set_user_seq(1).set_service_seq(999).set_service_module_seq(1).set_service_member_seq(1)
        .set_start_dt(start).set_end_dt(end);
    let a = draft("rust-u1").set_uuid(Some("rust-upsert")).set_read_count(1).bind(&db).insert().await.expect("insert a").unwrap();
    let b = draft("rust-u2").set_uuid(Some("rust-upsert")).set_read_count(1)
        ...
        .bind(&db).insert().await.expect("upsert").unwrap();
    check!(fails, a.seq == b.seq && b.name == "rust-u2" && b.read_count == 6, "on_duplicate: existing row returned, set + plus applied");
    let c = draft("rust-u3").set_uuid(Some("rust-upsert")).set_read_count(9).on_duplicate_set_all().bind(&db).insert().await.expect("upsert set_all").unwrap();
    check!(fails, c.seq == a.seq && c.name == "rust-u3" && c.read_count == 9, "on_duplicate_set_all copies the draft's set columns");
    let n = Battle::new().seq_eq(a.seq).plus_read_count(2).bind(&db).update().await.expect("query update");
    check!(fails, n == 1 && Battle::new().bind(&db).one_by_seq(a.seq).await.unwrap().unwrap().read_count == 11, "query update: affected 1, plus applied");
    let n = Battle::new().seq_eq(a.seq).minus_read_count(100).bind(&db).update().await.expect("query update minus");
    check!(fails, n == 1 && Battle::new().bind(&db).one_by_seq(a.seq).await.unwrap().unwrap().read_count == 0, "minus clamps at zero");
    let saved = Battle::new().set_seq(a.seq).set_name("rust-saved").bind(&db).save().await.expect("save update").unwrap();
    check!(fails, saved.seq == a.seq && saved.name == "rust-saved" && saved.uuid.as_deref() == Some("rust-upsert"), "save with pk: update + re-read");
    let inserted = draft("rust-saved-new").bind(&db).save().await.expect("save insert").unwrap();
    check!(fails, inserted.seq != a.seq && inserted.name == "rust-saved-new", "save without pk: insert");
    match Battle::new().set_name("x").bind(&db).update().await {
        Err(e) if e.code() == "IR_INVALID" => {}
        other => { fails += 1; eprintln!("FAIL: update without where not rejected: {:?}", other.err()); }
    }
    let n0 = statements.load(Ordering::Relaxed);
    let s = Battle::new().service_seq_eq(7).select_aes_hex_email().limit(0, 1).bind(&db).sql().await.expect("sql");
    ...
    check!(fails, Battle::new().seq_in(vec![a.seq, inserted.seq]).bind(&db).delete().await.expect("query delete") == 2, "query delete: affected count");

    let (svc, m1, m2, md) = db.transaction(|tx| async move {
        let s = Service::new().set_name("rust-svc").bind(&tx).insert().await?.unwrap();
        ...
        let md = ServiceModule::new().set_service_seq(s.seq).set_name("rust-mod").bind(&tx).insert().await?.unwrap();
    ...
    let row = Service::new().seq_eq(svc)
        ...
        .bind(&db).one().await.expect("service").expect("service row");
    ...
    check!(fails, ServiceMember::new().seq_in(vec![m1, m2]).bind(&db).count().await.unwrap() == 0, "delete_cascade removed the members");
    check!(fails, Service::new().seq_eq(svc).bind(&db).count().await.unwrap() == 0, "delete_cascade removed the service");
    check!(fails, ServiceModule::new().seq_eq(md).bind(&db).count().await.unwrap() == 1, "no_cascade_delete kept the module");
    check!(fails, ServiceModule::new().seq_eq(md).bind(&db).delete().await.unwrap() == 1, "module cleaned up");
    ...
    if driver != "sqlite" {
    let a = draft("dl-rust-1").bind(&db).insert().await.expect("dl a").unwrap().seq;
    let b = draft("dl-rust-2").bind(&db).insert().await.expect("dl b").unwrap().seq;
        ...
        tokio::spawn(async move {
            ...
            db.transaction(|tx| {
                ...
                async move {
                    ...
                    Battle::new().seq_eq(first).set_like_count(id as i64 * 10).bind(&tx).update().await?;
                    ...
                    Battle::new().seq_eq(second).set_like_count(id as i64 * 10).bind(&tx).update().await?;
    ...
    let (ra, rb) = (Battle::new().bind(&db).one_by_seq(a).await.unwrap().unwrap(), Battle::new().bind(&db).one_by_seq(b).await.unwrap().unwrap());
    check!(fails, expect > 0 && ra.like_count == expect && rb.like_count == expect, "final values are the last writer's");
    check!(fails, Battle::new().seq_in(vec![a, b]).bind(&db).delete().await.unwrap() == 2, "deadlock rows cleaned up");
    }

    // ---- S5: on_query plan_id, secret masking, driver error mapping, orm.toml ----
    let s = Battle::new().seq_eq(1).bind(&db).sql().await.expect("sql");
    Battle::new().seq_eq(2).bind(&db).all().await.expect("all"); // same shape (kind all), another value
    ...
    let d1 = draft("rust-dup").set_uuid(Some("rust-dup")).bind(&db).insert().await.expect("dup fixture").unwrap();
    // the driver's own message is kept
    let driver_msg = match driver.as_str() { "mysql" => "Duplicate entry", "postgres" => "23505", _ => "UNIQUE" };
    match draft("rust-dup-2").set_uuid(Some("rust-dup")).bind(&db).insert().await {
    ...
    check!(fails, Battle::new().seq_eq(d1.seq).bind(&db).delete().await.unwrap() == 1, "duplicate fixture cleaned up");
    ...
    let b = Battle::new().bind(&db2).one_by_seq(42).await.expect("one via from_config").expect("row 42");
