...
impl Engine {
    pub fn new(cfg: EngineConfig<'_>) -> Result<Engine> {
        ...
        ready_rx
            .recv()
            .map_err(|_| Error::Config("engine thread died during start".into()))??;
        let hash = {
            let v: serde_json::Value = serde_json::from_slice(cfg.schema_json)
                .map_err(|e| Error::Config(e.to_string()))?;
...
impl Inner {
    fn start(
        ...
        let memory = instance
            ...
            .ok_or_else(|| Error::Config("no memory export".into()))?;
        let func = |store: &mut wasmtime::Store<_>, name: &str| -> Result<wasmtime::Func> {
            instance
                .get_func(&mut *store, name)
                .ok_or_else(|| Error::Config(format!("missing export {name}")))
        };
        let alloc = func(&mut store, "orm_alloc")?
            .typed::<u32, u32>(&store)
            .map_err(cfg_err)?;
        let free = func(&mut store, "orm_free")?
            .typed::<u32, ()>(&store)
            .map_err(cfg_err)?;
        // orm_load_dialect(schema, dialect): the dialect travels in a second buffer (engine/wasm).
        let load = func(&mut store, "orm_load_dialect")?
            .typed::<(u32, u32, u32, u32), u32>(&store)
            .map_err(cfg_err)?;
        let compile = func(&mut store, "orm_compile")?
        ...
        let rp = load
            .call(
                &mut inner.store,
                (sp, schema.len() as u32, dp, dialect.len() as u32),
            )
            .map_err(cfg_err)?;
    ...
    fn put(&mut self, input: &[u8]) -> Result<u32> {
        let p = self
            ...
            .map_err(|e| Error::Config(e.to_string()))?;
        self.memory
            .write(&mut self.store, p as usize, input)
            .map_err(|e| Error::Config(e.to_string()))?;
        Ok(p)
    }
    ...
    fn take_result(&mut self, rp: u32) -> Result<(u32, Vec<u8>)> {
        ...
        let mut hdr = [0u8; 8];
        self.memory
            .read(&self.store, rp as usize, &mut hdr)
            .map_err(|e| Error::Config(e.to_string()))?;
        let status = u32::from_le_bytes(hdr[0..4].try_into().unwrap());
        let len = u32::from_le_bytes(hdr[4..8].try_into().unwrap()) as usize;
        let mut out = vec![0u8; len];
        self.memory
            .read(&self.store, rp as usize + 8, &mut out)
            .map_err(|e| Error::Config(e.to_string()))?;
    ...
    fn call(
        &mut self,
        f: &wasmtime::TypedFunc<(u32, u32), u32>,
        input: &[u8],
    ) -> Result<(u32, Vec<u8>)> {
        let wrap = |e: wasmtime::Error| Error::Config(e.to_string());
        let p = self.put(input)?;
        let rp = f
            .call(&mut self.store, (p, input.len() as u32))
            .map_err(wrap)?;
