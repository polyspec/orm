...
func assignServiceValue(r *ServiceRow, name string, v any) {
	switch name {
	case "seq":
		r.Seq = orm.AsInt64(v)
	case "name":
...
func acceptsServiceDirect(a *plan.Assemble) bool {
	if len(a.Columns) != 2 {
		return false
	}
...
func decodeServiceDirect(s *orm.DirectScanner, c plan.OutCol, raw *orm.ScanValue, r *ServiceRow) error {
	v, err := s.Decode(c, raw.Value())
	if err != nil {
		return err
	}
	assignServiceValue(r, c.Name, v)
	return nil
}

// scanServiceDirect scans the default flat projection into generated typed
// fields. Codec and datetime outputs use named temporary scan values.
func scanServiceDirect(s *orm.DirectScanner) (*ServiceRow, error) {
	a := s.Assemble()
	_ = a
	r := &ServiceRow{}
	r.Binding = s.Binding()
...
func scanService(vals []any, a *plan.Assemble, rs *orm.Rows) *ServiceRow {
	...
	for _, c := range a.Columns {
		assignServiceValue(r, c.Name, vals[c.Index])
	}
...
func (q *ServiceQuery) IfParentAesKeyVersionEq(v int32) *ServiceQuery {
...
func (q *ServiceQuery) GetOrNil() (*ServiceRow, error) {
	...
	q.q.Req.IR.Kind = "one"
	if direct, used, err := orm.QueryDirect(ctx, ex, q.q.Req, acceptsServiceDirect, scanServiceDirect); used {
...
func (q *ServiceQuery) Gets() (*orm.Collection[ServiceRow], error) {
	...
	if direct, used, err := orm.QueryDirect(ctx, ex, q.q.Req, acceptsServiceDirect, scanServiceDirect); used {
		...
		return collectServiceDirect(direct, q.keyFn), nil
	}
	rows, err := orm.Query(ctx, ex, q.q.Req)
	if err != nil {
		return nil, err
...
func collectServiceDirect(rows []*ServiceRow, keyFn func(*ServiceRow) orm.Key) *orm.Collection[ServiceRow] {
	c := orm.NewCollection[ServiceRow](len(rows))
	for _, r := range rows {
		if keyFn != nil {
			c.Put(keyFn(r), r)
		} else {
...
func (q *ServiceQuery) GetsAfter(cursor string, per int) (*orm.KeysetPage[ServiceRow], error) {
	...
	items := collectService(rows, q.keyFn)
...
func (q *ServiceQuery) GetsBefore(cursor string, per int) (*orm.KeysetPage[ServiceRow], error) {
...
func (q *ServiceQuery) BatchInsert(rows []*ServiceQuery, options orm.BatchOptions) (orm.BatchResult, error) {
...
func (q *ServiceQuery) BatchUpsert(rows []*ServiceQuery, options orm.BatchOptions) (orm.BatchResult, error) {
	return q.batchWrite(rows, "insert", options)
}

// BatchUpdate updates typed query drafts in one transaction.
func (q *ServiceQuery) BatchUpdate(rows []*ServiceQuery, options orm.BatchOptions) (orm.BatchResult, error) {
	return q.batchWrite(rows, "update", options)
}

// BatchDelete deletes typed query drafts in one transaction.
func (q *ServiceQuery) BatchDelete(rows []*ServiceQuery, options orm.BatchOptions) (orm.BatchResult, error) {
	return q.batchWrite(rows, "delete", options)
}
func (q *ServiceQuery) batchWrite(rows []*ServiceQuery, kind string, options orm.BatchOptions) (orm.BatchResult, error) {
	ctx, ex, err := q.binding.Resolve()
	if err != nil {
