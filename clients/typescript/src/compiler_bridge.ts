...
function group(value: Group | undefined, path: string): any {
  ...
  return {
    ...
    items: value.items.map((item, index) => {
      ...
      return { value: { case: 'navigation', value: { connector:item.nav!.conn??'', relation:item.nav!.rel, group:group(item.nav!.group,`${itemPath}.navigation.group`), mode:item.nav!.mode??'', countOperator:item.nav!.count_op??'', countParameter:item.nav!.p } } };
    }),
  };
}
