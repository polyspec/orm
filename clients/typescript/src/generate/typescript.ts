...
/** Whether a column has a json or jsons stage, whose value is an ordered-json value. */
function jsonStage(c: Column): boolean {
  return (c.styles ?? []).some(style => style === 'json' || style === 'jsons');
}

/** The type a setter takes. */
function tsSet(c: Column): string {
  if (jsonStage(c)) return c.nullable ? 'JsonValue | CodecValue | null' : 'JsonValue | CodecValue';
  const t = tsScalar(c);
...
function tsField(c: Column): string {
  if (jsonStage(c)) return c.nullable ? 'JsonValue | null' : 'JsonValue';
  let t: string;
  switch (category(c)) {
...
class Generator {
  ...
  public write(runtime: string): string {
    ...
    b += `import { CORE, Model, registerSchema, type CodecValue, type Collection, type ColumnFunction, type EntityDef, type EntitySchema, type JsonValue, type Point, type SchemaSet, type ValueFunction } from ${tsString(runtime)};\n\n`;
