...
Go transaction adapter는 정본 `schema.json` manifest를 `Tx.InstallSchema(context.Context, []byte) error`로 받는다. 연결된 transaction이 자신의 dialect를 선택하고 manifest의 schema engine을 공용 ORM database에 compile·등록한 뒤 ORM schema renderer를 통해 변환하고 caller-owned transaction 안에서 적용 가능한 문장을 실행한다. 소비자는 SQL·DDL 문자열·dialect 분기·driver transaction을 제공하지 않는다. 이 작업의 유일한 schema 입력은 manifest다.
## 공개 client 생성
