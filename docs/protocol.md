...
The Go and Rust database executors compile every plan-cache miss through `CompilerTransport`; startup rejects mismatched schema hash, dialect, and IR version metadata. Their 58 SQLite database vectors pass through Connect. PHP still uses the length-prefixed Unix socket and TypeScript has no database executor. These incomplete paths do not satisfy the completed transport requirement. T7.1 completes after all four executors use Connect and the 58 database vectors pass in all four languages.
