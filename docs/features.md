...
- `transactions`: Expose opt-in retry, isolation, read-only mode, savepoints, and row locks with capability errors. Query timeout and cancellation are not public until all clients provide the same behavior.
