...
- [ ] P9.4 Transaction option이 isolation, read-only mode, nested savepoint, query timeout/cancellation, 지원되는 row lock을 처리한다. 지원하지 않는 database mode는 정확한 capability error를 반환한다. PostgreSQL `timeoutMs`는 구현했으며, PHP PDO가 다른 client와 동일한 driver 동작을 제공할 수 없으므로 공통 계약에 실행 중 cancellation operation을 선언하지 않는다.
