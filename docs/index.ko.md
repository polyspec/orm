...
구현 클라이언트는 **Go, PHP, Rust, TypeScript**다. 네 클라이언트가 생성 entity API와 네이티브 데이터베이스 실행을 제공한다. 검사 범위와 잔여 작업은 [구현 대조표](interface-implementation.md), [체크리스트](checklist.md), [공개 소스 준비 계획](public-readiness.md), [S7 작업 목록](s7.md)에 기록한다.
