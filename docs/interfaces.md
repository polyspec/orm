...
Root row selects expose `forUpdate()`, `forShare()`, `forUpdateNoWait()`, and `forShareNoWait()` using each client's naming convention. The request stores `lock` in the common IR. MySQL and PostgreSQL append the selected lock clause after ordering and limits; `NoWait` fails immediately when the row is unavailable. SQLite implements `forUpdate()` and `forShare()` as transaction-level serialization: the ORM opens transactions with `BEGIN IMMEDIATE`, and emits no SQLite lock suffix. SQLite rejects the `NoWait` modes with `CAPABILITY_UNSUPPORTED` because they cannot be expressed after a transaction has begun.
...
Errors use the codes in [errors.yaml](errors.yaml). Go callers use `orm.ErrorCode`, `orm.IsDeadlock`, `orm.IsDuplicateKey` and `orm.IsForeignKey` for adapter-neutral classification; they do not inspect driver error types. Codec styles are schema declarations. The AES version column is non-null integer plaintext metadata with no encoding style and is excluded from the default projection. AES rotation updates all AES payload columns and the version in one transaction.
