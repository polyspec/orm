...
Errors preserve their stable code and the original driver message. A common query timeout/cancellation API is not public because the supported drivers do not provide one portable in-flight statement cancellation behavior. P9.4 remains incomplete; no client claims support.
