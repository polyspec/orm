...
- root row query는 `forUpdate()`, `forShare()`, `forUpdateNoWait()`, `forShareNoWait()`를 각 client의 명명 규칙으로 제공한다. MySQL과 PostgreSQL은 선택한 row lock을 실행하며 `NoWait`은 row를 즉시 사용할 수 없으면 실패한다. SQLite는 모든 row lock mode에 `CAPABILITY_UNSUPPORTED`를 반환한다.
