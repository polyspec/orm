...
Reads select the key from the stored `aes_key_version`. Configure every stored version in `secrets.aes_keys` before rotation. Set the new version as current for writes, run status, rotate in bounded batches, require `pending = 0`, verify the database, and remove the old key only after the retention period.
