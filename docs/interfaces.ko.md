...
root row select는 `forUpdate()`, `forShare()`, `forUpdateNoWait()`, `forShareNoWait()`를 각 언어의 명명 규칙으로 제공한다. request는 공통 IR에 `lock`을 저장한다. MySQL과 PostgreSQL은 order와 limit 뒤에 선택한 lock 절을 추가하며 `NoWait` mode는 row를 즉시 사용할 수 없으면 실패한다. SQLite는 모든 row lock mode를 `CAPABILITY_UNSUPPORTED`로 거부하며 client가 row lock을 대체하지 않는다.
