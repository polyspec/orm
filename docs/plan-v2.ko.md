...
| 3 | 더 나은 방식·문법 | 접근은 유지(컴파일러 1 + 네이티브 실행기 + 형태 캐시). 문법: `gtCreatedTs($t)` **유지**, `or<op><Col>` 접두어 **삭제**, 관계는 스키마 선언 기반 `withItems(child)`, `raw()`→`expr()`(스키마 검사 조각), 텍스트 DSL·리터럴 필터·빌드타임 SQL 기각 | 생성량 절감. 텍스트 DSL은 `if` 분기 조립에서 문자열 결합으로 퇴행 |
