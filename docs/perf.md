...
같은 프로세스에서 생성 클라이언트와 `bench/go`의 손으로 쓴 네이티브 문장을 각각 300회 p50으로 재고 비율을 본다(장비가 달라도 비교 가능). 현재: PK 0.48배(클라이언트가 더 빠름 — 네이티브 헬퍼가 호출마다 스캔 대상을 다시 만든다), 100행 1.03배. 한계선 PK 0.90 / 100행 1.25이며, 넘으면 CI가 실패하고 "원인을 설명하거나 perf.md에서 한계선을 옮기라"고 말한다. 한계선을 조용히 올리지 않는다.
