...
`db.plan_cache_size`와 `db.statement_cache_size`는 양의 정수 제한값이다. 두 값의 기본값은 256이다. 모든 client는 제한 초과 시 먼저 삽입된 항목을 제거한다. PHP는 process-local plan cache와 PDO statement에 제한을 적용한다. TypeScript는 statement 제한을 mysql2에 전달하고, PostgreSQL에는 연결별 named statement 제한을 적용하며, SQLite에는 `StatementSync` 제한을 적용한다. `close`는 client가 소유한 cache를 비우고 driver가 제공하는 database 자원을 닫는다.
