...
1. `agg_min_max` — `Battle.serviceSeq(7).bind(db).minSeq()`, `.bind(db).maxSeq()`, `.bind(db).countDistinctUserSeq()` → `{"min": 6, "max": 99906, "distinct_users": <n>}` (n is whatever the data gives; identical across languages).
2. `group_count_having` — `Battle.serviceSeq(7).groupByUserSeq().having(w => w.expr("COUNT(*) > ?", [1])).bind(db).getCount()` → number of users with more than one battle in service 7.
3. `predicate_named` — `Battle.visible().serviceSeq(7).bind(db).getCount()` and `Battle.startedAfter("2026-01-01 00:00:00").serviceSeq(7).bind(db).getCount()` → `{"visible": …, "started_after": …}`.
4. `raw_root` — `Battle.raw("SELECT COUNT(*) AS n, MAX(seq) AS m FROM {table} WHERE service_seq = ? AND is_close = ?", [7, 0]).bind(db).rawAll()` → `[{"n": …, "m": …}]` (ints as numbers).
