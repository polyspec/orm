...
Engine contract (committed): IR kinds `count_distinct`, `min`, `max` (with `agg`), `having: Group`
(root only, needs `group_by`; aggregate expressions go through `expr` items), `count` + `group_by` =
number of groups, `kind: raw` with `raw: {sql, ps}` (step role `raw`, no assemble), manifest
...
1. `agg_min_max` — `Battle.serviceSeqEq(7).minSeq(db)`, `.maxSeq(db)`, `.countDistinctUserSeq(db)` → `{"min": 6, "max": 99906, "distinct_users": <n>}` (n is whatever the data gives; identical across languages).
2. `group_count_having` — `Battle.serviceSeqEq(7).groupByUserSeq().having(w => w.expr("COUNT(*) > ?", [1])).count(db)` → number of users with more than one battle in service 7.
3. `predicate_named` — `Battle.visible().serviceSeqEq(7).count(db)` and `Battle.startedAfter("2026-01-01 00:00:00").serviceSeqEq(7).count(db)` → `{"visible": …, "started_after": …}`.
4. `raw_root` — `Battle.raw("SELECT COUNT(*) AS n, MAX(seq) AS m FROM {table} WHERE service_seq = ? AND is_close = ?", [7, 0]).rawAll(db)` → `[{"n": …, "m": …}]` (ints as numbers).
