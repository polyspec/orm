...
- [~] T7.1 Generate typed Protobuf messages for Go, PHP, Rust, and TypeScript; provide the Connect compiler server and `CompilerTransport` implementations; and run the shared compiler transport vector. Switching each database executor to this transport and running all 58 database vectors remain.
