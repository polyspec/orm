...
- [~] T7.13 AES version-column validation and row rotation helpers exist for Go/PHP/Rust/TypeScript. `aes_key_version` is plaintext metadata, cannot use an AES style, and is excluded from the default projection. Go row persistence, transaction APIs, status reporting, and cross-language database checks remain
