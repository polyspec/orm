...
- [x] T4.7 `ormgen check --lang php`(호환층이 번역해야 할 토큰 사용 빈도 보고) + `--lang go` 프래그먼트 analyzer(백틱 컬럼 존재·`?`/바인드 개수, `ormgen:ignore` 주석으로 의도적 음성 테스트 제외) — CI 단계로 포함. 보고서는 실행하는 쪽 코드베이스에 남기고 이 저장소에는 두지 않는다
