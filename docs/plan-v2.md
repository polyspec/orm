...
| # | 질문 | 결정 | 근거 한 줄 |
|---|---|---|---|
| 1 | 왜 언어별 실행기인가, 프록시는 왜 버렸나 | ~~Go·Rust = 네이티브, PHP = `ormd` 사이드카가 실행+조립~~ → **S0 실측으로 철회: 세 언어 모두 네이티브 실행기, `ormd`는 컴파일 전용** (`docs/perf.md` §5: ormd 실행 경로가 PDO 대비 PK +69%, 목록 +152%, 4단 관계 +6%) | 행이 경계를 넘는 비용이 홉보다 크고, Go 조립이 PHP 조립보다 빠르지 않았다. 드리프트 위험은 적합성 벡터(키 타입 태그 등)로 잡는다 |
| 2 | join의 addColumn/where 겹침, `'('` 괄호 함정 | **트리 WHERE**: 모델 체인 = 그룹, 조인 체인 = 부모에 AND로 붙는 그룹, 같은 모델 중첩은 `and(f)/or(f)`, 모델 경계는 `cols()` 술어 값(`andPred/orPred`). 괄호 토큰·스플라이스·균형 검증 삭제. 컬럼은 위치 매핑 + alias 네임스페이스 + `addColumn<Col>As(name)` | 침묵 실패가 없는 유일한 조합. 실코드 (a)~(f) 전부 표현 가능 |
| 3 | 더 나은 방식·문법 | 접근은 유지(컴파일러 1 + 네이티브 실행기 + 형태 캐시). 문법: `gtCreatedTs($t)` **유지**, `or<op><Col>` 접두어 **삭제**, 관계는 YAML 선언 기반 `withItems(child)`, `raw()`→`expr()`(스키마 검사 조각), 텍스트 DSL·리터럴 필터·빌드타임 SQL 기각 | compatibility 느낌 유지 + 생성량 절반 + 폴백 옵션 제거. 텍스트 DSL은 `if` 분기 조립에서 문자열 결합으로 퇴행 |

## 1. 실행 위치 (Q1) — **S0 실측 후 철회됨. 아래 본문은 검수 당시 안이며, 확정안은 `docs/perf.md` §5·§7 (R3).**

```
Go   앱 ── in-process engine + Go 실행기(database/sql) ──▶ DB        홉 0
Rust 앱 ── engine 경계(S0 확정: libloading .so | wasmtime .wasm) + Rust 실행기(sqlx) ──▶ DB   홉 0
...
- DSN은 `ormd`만 보유. PHP 워커에서 DB 자격 제거(신뢰 경계 축소). PHP의 raw SQL은 `Query.raw`로 `ormd` 경유. 워커 로컬 PDO 금지.
- 와이어: S0에서 JSON vs msgpack(PHP ext) 실측 후 하나 확정.
- 성능 게이트 분리: Go/Rust 핫패스 네이티브 대비 처리량 손실 ≤5%·CPU ≤+10%. PHP는 원격 DB 단건 ≤+25%, 100행 ≤+15%, 4단 관계는 **compatibility보다 빠를 것**(Go 조립 > PHP 조립 가설을 S0에서 검증).
- 뒤집는 조건(문서화): PHP가 FrankenPHP 워커 모드 → in-process 승격. PHP 주력화 또는 S0에서 "PDO+PHP 조립"이 "ormd+디코드"보다 빠르면 PHP 네이티브 실행기로 복귀.
- S4 추가: 플래너 옵션 `multi_statement` — 관계 단계 N개를 한 왕복으로(Go `NextResultSet`, PHP `nextRowset`, sqlx `fetch_many`). 원격 DB에서 프록시 없이 RTT 절감.
...
4. `andPred(p)` / `orPred(p)`: **술어 값**. `$x->cols()`(Go `x.Cols()`, Rust `x.cols()`)가 alias가 박힌 읽기 전용 typed 컬럼 참조를 주고, 그 메서드가 `Pred`를 만든다. `Pred::all([...])`, `Pred::any([...])`, `Pred::not(p)`. **모델 경계를 넘는 유일한 통로.** 참조 alias가 문장에 없으면 `AliasUnknown`.
...
### 시나리오 (a) — 키워드를 여러 조인 테이블에서 OR 검색 (실코드 ProductReview/SearchFromGet)
```php
...
```
```go
ga1 := m.NewProductBrandLang().Alias("ga1");  ga2 := m.NewProductBrand().Alias("ga2");  ga3 := m.NewProductLang().Alias("ga3")
products, err := m.NewProduct().
    WithLang(m.NewProductLang().LangId(langId)).
...
$battles = (new Battle)->addAllColumns()->withItems((new BattleItem)->orderByOrderNumber())
    ...
    ->orderBySeqDesc()->paginate($db, $page, 20);
```
이런 재사용 조건은 YAML `predicates:`로 선언해 `->displayCondition()` 메서드로 생성한다(S4).
...
- 충돌은 컴파일 에러(`ColumnAliasConflict`): `parentNode()` 병합 컬럼이 루트 컬럼(PK 제외)과 같음, 같은 조인 안 별칭 중복, 두 parentNode 조인 간 중복. YAML 검증에 `__` 금지 추가, 엔진 내부 SELECT 별칭은 `alias__col`.

### PHP 호환층
- 모델 안에서 균형 잡힌 `and('(')/or('(')/condition('(')…condition(')')`, `->{'condition(AAndB)Or(C)'}`는 파싱 시 스택으로 트리화(루트 중첩 301/271, 조인 자체 34/3 → 사실상 전부).
- 모델 경계를 넘는 괄호(조인에서 닫힘 ≈31건, 조인 첫 술어 `or*` 26건)는 `ParenAcrossModels` 에러 + `orPred($alias->cols()->…)` 재작성 힌트. `ormgen check --lang php`가 목록을 출력.

## 3. 문법 결정 (Q3)

- **유지**: `new Entity`, `<col>(v)`(`=`), `<op><Col>(v)`(`ne gt ge lt le in nin lk lb between isNull notNull fulltext… fulltextBoolean…`), `getBy<PK|unique>`, `set*/setRaw*/plus*/minus*`, `keyName<Col>() parentNode() groupLimit(n) possible<Col>(v) stripKey()`, 정렬·그룹·limit·컬럼 선택, 터미널·트랜잭션·결과 접근·컬렉션 규칙(v1 표).
...
- **추가**:
  - `with<Rel>(child)` — YAML `relations:` 선언 관계 부착. kind(one/many)·left/right는 YAML이 안다.
  - `join<Rel>(child)` / `leftJoin<Rel>(child)` — 선언 관계 조인. `child.on(f)`가 ON. 미선언 조인은 YAML에 선언하거나(권장) `join(alias, Col.cmp(Col), child)` 탈출구.
  ...
  - YAML `predicates:` — 이름 붙인 재사용 술어 그룹 → `<name>()` 메서드 생성 (S4).
...
- `ormgen check --lang php,go`: PHP 레거시 이름·`expr` 백틱 컬럼·`ParenAcrossModels`를 스키마 대조(PHP의 타입 검사 역할). Rust는 컴파일러. S5 CI 게이트.
- 기각 이유 기록: 텍스트 DSL(if 분기 조립에서 문자열 결합 퇴행, IDE/타입 상실, PHP 오타가 런타임 에러), 구조체/맵 리터럴(Go/Rust `Option/Default` 범벅, 술어 순서 비결정), 빌드타임 SQL 생성(분기 조합 폭발 → 린트로만), E′ 전면 컬럼 객체(compatibility 느낌 손실이 생성량 절감보다 큼).
...
- 데드락은 벡터가 아니라 3언어 공통 동시성 테스트(2 tx 교차 갱신)로 별도 게이트.
- S0 항목 추가: PHP 3경로 실측 — (i) PDO+PHP 조립(비교 기준, 구현하지 않고 compatibility로 측정), (ii) `ormd`+JSON, (iii) `ormd`+msgpack — 단건·100행·4단 관계, 로컬/원격.

## 6. 마일스톤 조정
- S0(3일): Rust 경계 확정, PHP 와이어 확정, PHP 3경로 수치.
...
- S4: `join<Rel>`·`join(alias, cmp, child)`, `cols()`·`orPred`, `expr`, YAML `predicates:`, `multi_statement`, `ormgen check --lang php` 목록.
- 총 기간 변화 없음(≈15–16주). PHP 조립·코덱·데드락 구현이 빠진 만큼 `ormd` 커넥션 핀·프레이밍이 들어온다.

## 7. v1에서 그대로 유지
컴파일러 전용 엔진, 형태 해시 플랜 캐시(Go/Rust 프로세스 내, PHP는 APCu에 IR→응답이 아니라 **IR 해시→ormd 플랜 id**), YAML 스키마·`import/validate`·`schema_hash`, Relation 의미론(배치 IN, keyName 재키잉, parentNode 병합 순서, ONE 첫 행, groupLimit 파티션, possible strict), MutationBatch·낙관 락·`CLIENT_FOUND_ROWS`, 결과 접근·컬렉션 규칙, 에러 enum 생성, 코덱 벡터, 설계 원칙 3개, S0 스파이크 골격, `--tables`.
