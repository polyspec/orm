...
| 2 | join의 addColumn/where 겹침, `'('` 괄호 함정 | **트리 WHERE**: 모델 체인 = 그룹, 조인 체인 = 부모에 AND로 붙는 그룹, 같은 모델 중첩은 `and(f)/or(f)`, 모델 경계는 `cols()` 술어 값(`andPred/orPred`). 괄호 토큰·스플라이스·균형 검증 삭제. 컬럼은 위치 매핑 + alias 네임스페이스 + `addColumn<Col>As(name)` | 침묵 실패가 없는 유일한 조합. 시나리오 (a)~(f) 전부 표현 가능 |
...
- 성능 게이트 분리: Go/Rust 핫패스 네이티브 대비 처리량 손실 ≤5%·CPU ≤+10%. PHP는 원격 DB 단건 ≤+25%, 100행 ≤+15%, 4단 관계는 **PDO 기준선 이하**(Go 조립 > PHP 조립 가설을 S0에서 검증).
