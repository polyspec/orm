...
`db.plan_cache_size` and `db.statement_cache_size` are positive integer limits. The default for both is 256. Go and Rust remove the oldest inserted entry when a limit is exceeded. PHP applies the plan limit to its request cache and the statement limit to PDO statements. TypeScript passes the statement limit to mysql2, applies a per-connection named statement limit to PostgreSQL, and applies a `StatementSync` limit to SQLite. `close` clears client-owned caches and closes the database resource where the driver provides that operation.
