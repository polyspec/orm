...
`db.plan_cache_size` and `db.statement_cache_size` are positive integer limits. The default for both is 256. Every client removes the oldest inserted entry when a limit is exceeded. PHP applies the limits to its process-local plan cache and PDO statements. TypeScript passes the statement limit to mysql2, applies a per-connection named statement limit to PostgreSQL, and applies a `StatementSync` limit to SQLite. `close` clears client-owned caches and closes the database resource where the driver provides that operation.
