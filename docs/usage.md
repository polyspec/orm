...
- Root row queries provide `forUpdate()`, `forShare()`, `forUpdateNoWait()`, and `forShareNoWait()` using each client's naming convention. MySQL and PostgreSQL execute the selected row lock; `NoWait` fails immediately when the row is unavailable. SQLite implements `forUpdate()` and `forShare()` with transaction-level serialization through `BEGIN IMMEDIATE`; the `NoWait` modes return `CAPABILITY_UNSUPPORTED`.
