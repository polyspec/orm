...
The diff orders foreign-key removal before dependent index removal, applies column changes next, and creates indexes before foreign keys. It compares named indexes, unique constraints, full-text indexes, foreign-key targets and delete actions, column types, nullability, defaults, and comments. PostgreSQL emits separate `TYPE`, `SET|DROP NOT NULL`, and `SET|DROP DEFAULT` operations. Foreign keys use deterministic `fk_<table>_<column>` names and explicitly use `RESTRICT`, `CASCADE`, or `SET NULL`. SQLite changes that require replacing a table return an error until the verified rebuild plan described in the public source readiness plan is implemented.
