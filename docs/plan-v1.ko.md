...
| 검수 지적 | 채택한 수정 |
|---|---|
| 엔진이 실행을 맡으면 행 데이터가 언어 호출 구간를 두 번 넘고(직렬화), PHP-FPM에서 Go c-shared는 fork 후 로드·워커당 Go 런타임(스레드/RSS 4–8GB)·`ffi.enable` 문제, WASM은 요청마다 재컴파일 → PHP에서 FFI/WASM 실행은 불가. 행을 wasm으로 넘기면 100행 목록에서 예산 10배 초과 | **엔진 = 컴파일러만**(IR→Plan). 실행·풀·트랜잭션은 각 언어 네이티브 드라이버(database/sql, sqlx, PDO). 행 데이터는 절대 호출 구간를 넘지 않음 |
...
3. **폴백은 정말 필요한 곳에만.** 언어별 호출 구간 방식은 S0에서 **하나로 확정**하고 대안 경로를 코드에 남기지 않는다. 선언되지 않은 관계 alias는 "untyped bag" 폴백이 아니라 **검증 에러**. 스키마 불일치·컬럼 미지·괄호 불균형은 조용한 우회 없이 에러. 유일한 의도적 대안: `ormgen gen --no-or-prefix`(S2 컴파일 시간 검사 단계에서 한 번 결정하는 생성 옵션이며 런타임 폴백이 아님).
...
- 엔진 호출 구간는 형태당 1회. 캐시 키 = IR 형태 해시(값 제외, IN 카디널리티 포함) + `schema_hash`.
...
- RAW/`setRaw`/`lhs_expr`/raw order·group은 신뢰 코드 전용(호출자가 이미 DB 자격을 가짐); 바인드만 값 채널; `ormd`는 실행하지 않으므로 신뢰 호출 구간 확장 없음.
...
- 성능: 3계층 — (0) 엔진 `Compile` ns/op·allocs, (1) 호출 구간 에코(1KB/64KB), (2) e2e: MySQL 로컬 소켓 + 원격 호스트 각각, 워크로드 PK 단건·100행(aes_hex 2컬럼)·4단 관계·INSERT·3문 tx, 동시성 1/16/64, 워밍업 30s, A/B 교차 5회, p50/p99/p99.9·처리량·CPU-time/op·allocs·PHP 워커 RSS/스레드. **캐시 웜(핫패스)과 콜드를 분리 보고**. 검사 단계: 핫패스 네이티브 대비 처리량 손실 ≤5%·CPU-time ≤+10%(조립 비용), 콜드 형태당 컴파일 ≤1ms(Go) / ≤2ms(Rust 호출 구간) / ≤3ms(PHP UDS). CI에서 회귀 실패.
