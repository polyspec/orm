...
- [ ] P9.4 Transaction options cover isolation, read-only mode, nested savepoints, query timeout/cancellation, and supported row locks. Unsupported database modes return exact capability errors. PostgreSQL `timeoutMs` is implemented; the common contract intentionally declares no in-flight cancellation operation because PHP PDO cannot provide the same driver behavior as the other clients.
