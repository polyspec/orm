...
- [x] P9.4 Transaction options cover isolation, read-only mode, nested savepoints, query timeout, and supported row locks. Unsupported database modes return exact capability errors. PostgreSQL `timeoutMs` is implemented. In-flight cancellation is deliberately excluded from the common API because PHP PDO cannot provide the same driver behavior as the other clients; prohibited-symbol checks prevent a client-specific public cancellation method.
  - [x] P9.4a Root `forUpdate`, `forShare`, `forUpdateNoWait`, and `forShareNoWait` row locks use the common IR. MySQL/PostgreSQL execute the selected row lock; SQLite implements all four modes with an ORM transaction-scoped database lock row, and `NoWait` uses zero busy timeout for immediate contention failure. This sub-item does not complete P9.4 because timeout/cancellation remains.
...
- [x] P10.1 Model and property tests cover schema round trips, migration operation order, cursor round trips, query cloning, and codec round trips. The local targeted test set and 100-case manifest round-trip property test passed after the schema and generator updates.
