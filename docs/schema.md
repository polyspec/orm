...
- PostgreSQL (`--driver postgres`, or a `postgres://` DSN) reads `information_schema.columns`, `pg_index`, and `pg_constraint` and writes the same diagram. It normalizes types (`character varying(191)`→`varchar(191)`, `boolean`→`tinyint`, `numeric(p,s)`→`decimal(p,s)`, `timestamp(6)`→`datetime(6)`, `inet`→`varbinary(16)`, `jsonb`→`json`) and maps identity/`nextval` to `auto`. Live import does not yet reconstruct expression indexes, including generated GIN full-text indexes; use Mermaid, manifest, or ORM SQL metadata as the source when those indexes are present.
