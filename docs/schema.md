...
%% aes_version <table> <column>           # AES payloads use this non-null integer version column
...
The `orm:table` directive is the only schema-source declaration for a qualified physical table. The manifest preserves both identifier components. PostgreSQL DDL and query quoting treat the schema and table as separate identifiers; SQLite preserves the namespace by mapping `schema.table` to the deterministic physical name `schema__table`, including the same namespace in generated index names. This prevents two module schemas with the same table base name from colliding in one SQLite database.
