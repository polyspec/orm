...
- [ ] T7.21 선언형 `soft_delete` schema directive를 추가하고 nullable datetime column을 검증하며 read·update에 활성 행 predicate를 적용하고 delete를 timestamp update로 변환한 뒤 네 client의 물리 DB를 검증한다.
