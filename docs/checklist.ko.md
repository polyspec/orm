...
- 현재 적합성 범위는 **58개 벡터 × 3개 클라이언트 × 3개 데이터베이스**다. 코덱 범위는 3개 클라이언트 60개 벡터다. TypeScript는 현재 구조 검사만 수행한다.
...
- [~] T2.15 결정적인 150테이블 Rust fixture를 생성·컴파일하고 결과를 기록한다. fixture와 `make rust-150-check` 명령은 추가했으며 Cargo 실행 결과가 대기 중이다.
...
- [~] T7.8 TypeScript 구조·AST 검사를 구현했다. `make typescript-build`로 모듈을 빌드할 수 있으며 TypeScript 실행기와 공통 벡터 실행이 남아 있다.
...
- [~] T7.12 결정적인 150테이블 Rust fixture와 재현 검사 명령을 추가했다. Cargo 컴파일 결과가 대기 중이다.
- [~] T7.13 AES version column 검사와 행 재암호화 도우미를 구현했다. 데이터베이스 저장, 상태 조회, 4개 클라이언트의 동등한 DB API가 남아 있다.
- [x] T7.14 매니페스트, 스키마 해시, import, DDL, diff, SQLite 메타데이터에 테이블·컬럼 주석을 포함했습니다. 단위 테스트가 통과했으며 물리 데이터베이스 검증은 T7.18에 남아 있습니다.
- [~] T7.15 MySQL, PostgreSQL, SQLite의 마이그레이션 실행 잠금, 트랜잭션 처리 범위, 상세 복구 상태를 추가합니다.
- [~] T7.16 세미콜론 분할을 방언별 SQL 문 분석기로 교체하고 문장별 실패 위치를 보존합니다.
- [ ] T7.17 MMD, SQL, JSON, 실제 DB 사이의 마이그레이션 입력 변환을 모든 방향으로 제공하고 손실과 롤백 계획을 명시합니다.
- [~] T7.18 containerctl로 MySQL·PostgreSQL의 주석, 계획 적용, 반복 실행, drift, 실패, 복구를 물리 DB에서 검증합니다.
...
- [~] G2 150테이블 Rust compile 검사를 완료한다. 재현 명령은 추가했으며 로컬에 Cargo가 없다.
- [x] G3 구현된 클라이언트의 쓰기·관계 벡터를 검사한다.
- [x] G4 생성 심볼·스키마·CI 검사를 실행한다.
- [x] G5 GitHub Actions 빌드를 확인한다.
- [ ] G7 T7.1~T7.13과 T7.D1~T7.D10의 완료 조건을 모두 충족한 뒤 닫는다.
