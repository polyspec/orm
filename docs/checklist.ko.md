...
- [~] T7.15 MySQL, PostgreSQL, SQLite의 마이그레이션 실행 잠금, 트랜잭션 처리 범위, 상세 복구 상태를 추가합니다. 트랜잭션 실행, rollback 결과, 실제 잠금 충돌 검사가 통과했으며 명시적인 복구 작업이 남아 있습니다.
...
- [~] T7.18 containerctl로 MySQL·PostgreSQL의 주석, 계획 적용, 반복 실행, drift, 실패, 복구를 실제 DB에서 검증합니다. 주석, 최초 적용, 반복, 잠금 충돌은 통과했으며 계획 적용, drift, 실패, 복구가 남아 있습니다.
