...
| PHP runtime | `ormd` daemon over a persistent unix socket + APCu plan cache | compile round trip ~50µs once per shape; hot path is PDO only (perf.md §6) | FrankenPHP/in-process PHP (S7): then the engine could be loaded once per worker without fork issues |
