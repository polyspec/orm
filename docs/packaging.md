...
| PHP runtime | `ormd` daemon over a persistent Unix socket + APCu plan cache | compile round trip ~50µs once per shape; hot path is PDO only (perf.md §6) | FrankenPHP/in-process PHP remains a separate runtime implementation |
